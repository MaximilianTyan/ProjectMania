...
public abstract class ModConditionnalMixinPlugin implements IMixinConfigPlugin {
    public static boolean modPresent = false;
    public Logger LOGGER = getLogger();

    public Logger getLogger() {
        return ModMain.LOGGER;
    }
    ...
    public abstract String getTargetModId();

    public abstract String getTargetModName();

    public abstract boolean shouldApplyMixinsWhithMod();
    ...
    public void onLoad(String mixinPackage) {
        modPresent = FabricLoader.getInstance().isModLoaded(getTargetModId());

        if (modPresent) {
            LOGGER.info("Mod {} (id: '{}') is {}", getTargetModName(), getTargetModId(), modPresent ? "present" : "absent");
        }

        if (isEnabled()) {
            LOGGER.info(
                    "{} associated mixins for mod {} (id: '{}')",
                    shouldApplyMixinsWhithMod() ? "Enabling" : "Disabling",
                    getTargetModName(),
                    getTargetModId()
            );
        } else {
            LOGGER.warn("Ignoring associated mixins for mod {} (id: '{}'): Disabled by Plugin",
                    getTargetModName(),
                    getTargetModId()
            );
        }
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (!shouldApplyMixinsWhithMod()) {
            return false;
        }

        if (modPresent) {
            return shouldApplyMixinsWhithMod();
        } else {
            return !shouldApplyMixinsWhithMod();
        }
    }
